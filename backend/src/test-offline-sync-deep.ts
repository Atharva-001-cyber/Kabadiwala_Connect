...
async function testOfflineSyncDeep() {
  ...
  const loginRes: any = await fetch(`${BASE}/auth/verify-otp`, {
    method: 'POST',
  ...
  const syncRes: any = await fetch(`${BASE}/lots/sync-batch`, {
    method: 'POST',
    headers: {
  ...
  const listRes: any = await fetch(`${BASE}/lots`, {
  ...
  const retrySyncRes: any = await fetch(`${BASE}/lots/sync-batch`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
  ...
  const listAfterRetry: any = await fetch(`${BASE}/lots`, {
    headers: { 'Authorization': `Bearer ${token}` }
  }).then(r => r.json());
  ...
  const traceRes: any = await fetch(`${BASE}/traceability/${saved.id}`).then(r => r.json());
  const logs = traceRes.timeline || [];
  console.log(`  Traceability logs found for lot: ${logs.length}`);
  const syncLog = logs.find((t: any) => t.title.includes('Offline Lot Synchronized'));
  if (!syncLog) {
  ...
  const col2Login: any = await fetch(`${BASE}/auth/verify-otp`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
