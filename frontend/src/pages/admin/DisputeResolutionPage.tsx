  ...
  return (
    <div className="space-y-6 pb-16">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              ...
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold tracking-wider bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 uppercase">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={loading}
              className="flex items-center gap-1.5 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white rounded-xl text-xs font-bold border border-slate-700 shadow-sm transition-all"
            >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
        {/* Total Disputes */}
        <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 shadow-lg hover:border-slate-700 transition-all">
        ...
        <div className="bg-slate-900 border border-rose-900/40 rounded-2xl p-4 shadow-lg hover:border-rose-700/60 transition-all bg-gradient-to-br from-rose-950/20 to-transparent">
          ...
          <div className="text-2xl sm:text-3xl font-black text-rose-400">{stats.openCount}</div>
        ...
        {/* Under Review */}
        <div className="bg-slate-900 border border-amber-900/40 rounded-2xl p-4 shadow-lg hover:border-amber-700/60 transition-all bg-gradient-to-br from-amber-950/20 to-transparent">
          ...
          <div className="text-2xl sm:text-3xl font-black text-amber-400">{stats.underReview}</div>
          <div className="text-[10px] text-slate-500 mt-1 font-medium">
        ...
        {/* Tare Arbitrations */}
        <div className="bg-slate-900 border border-cyan-900/40 rounded-2xl p-4 shadow-lg hover:border-cyan-700/60 transition-all bg-gradient-to-br from-cyan-950/20 to-transparent">
          ...
          <div className="text-2xl sm:text-3xl font-black text-cyan-400">{stats.tareArbitrations}</div>
          <div className="text-[10px] text-slate-500 mt-1 font-medium">
        ...
        {/* Settled & Closed */}
        <div className="bg-slate-900 border border-emerald-900/40 rounded-2xl p-4 shadow-lg hover:border-emerald-700/60 transition-all col-span-2 lg:col-span-1 bg-gradient-to-br from-emerald-950/20 to-transparent">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-700/80 rounded-2xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-all"
            />
            {searchQuery && (
              <button
            ...
            {[
              ...
              >
                ...
                <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-mono ${
                  statusFilter === tab.id ? 'bg-indigo-700 text-white' : 'bg-slate-900 text-slate-400'
                }`}>
                  {tab.count}
                </span>
              </button>
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-12 text-center text-slate-400 text-xs font-medium space-y-3">
          ...
          filteredDisputes.map((d) => (
            <div
              key={d.id}
              className="bg-slate-900 border border-slate-800/90 hover:border-slate-700/80 rounded-3xl p-5 sm:p-6 shadow-xl space-y-4 text-xs transition-all relative overflow-hidden"
            >
              {/* Top Case Meta Bar */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800/80 pb-3">
                <div className="flex items-center gap-2 flex-wrap">
                  {/* Dispute ID Badge */}
                  <span className="font-mono font-bold text-cyan-400 bg-cyan-950/60 border border-cyan-800/60 px-2.5 py-1 rounded-xl text-[11px]">
                    {d.id}
                  </span>

                  {/* Lot ID Badge */}
                  <span className="font-mono font-bold text-indigo-300 bg-indigo-950/60 border border-indigo-800/60 px-2.5 py-1 rounded-xl text-[11px] flex items-center gap-1">
                    <span>📦</span>
                  ...
                  <span className={`px-3 py-1 rounded-xl font-bold text-[11px] flex items-center gap-1.5 ${
                    d.status === 'RESOLVED'
                      ...
                      : 'bg-rose-950/80 text-rose-300 border border-rose-700/80 animate-pulse'
                  }`}>
                    {d.status === 'RESOLVED' ? (
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-3 text-slate-400 text-[11px] flex-wrap">
                      {d.recyclerContact && d.recyclerContact !== 'N/A' && (
                        <span>Contact: <b className="text-slate-300">{d.recyclerContact}</b></span>
                      )}
              ...
              {(d.resolutionNotes || d.adminNotes || d.status === 'RESOLVED') && (
                <div className="bg-emerald-950/30 p-4 rounded-2xl border border-emerald-800/60 space-y-2">
              ...
              <div className="pt-2 border-t border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 self-end">
                  <button
                    ...
                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl flex items-center gap-1.5 shadow-md shadow-indigo-600/30 transition-all text-xs"
                  >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-xl w-full p-6 shadow-2xl space-y-5 relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              ...
              <button
                ...
                className="p-1.5 text-slate-400 hover:text-white rounded-xl bg-slate-800 hover:bg-slate-700 transition-all"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800 space-y-2 text-xs">
              <div className="flex items-center justify-between">
                <span className="text-slate-400 font-bold uppercase text-[10px]">CLAIM REASON</span>
                <span className="text-slate-400">
                  Claimant: <b className="text-white">{activeModalDispute.collectorName || 'Collector'}</b>
                </span>
              </div>
              <p className="text-white font-medium">{activeModalDispute.reason}</p>
            </div>

            {/* Status Selection */}
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-300 block uppercase tracking-wider text-[11px]">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <button
                  ...
                  className="text-left p-2 rounded-xl bg-slate-950 hover:bg-slate-800/80 border border-slate-800 hover:border-indigo-500/50 text-[10px] text-slate-300 transition-all space-y-0.5"
                >
                  <div className="font-bold text-indigo-400">⚖️ Digital Tare Calibration Settlement</div>
                  <p className="text-slate-400 line-clamp-1">Legal Metrology certified calibration verified on site.</p>
                </button>
                ...
                <button
                  ...
                  className="text-left p-2 rounded-xl bg-slate-950 hover:bg-slate-800/80 border border-slate-800 hover:border-indigo-500/50 text-[10px] text-slate-300 transition-all space-y-0.5"
                >
                  <div className="font-bold text-emerald-400">💳 Instant Payout Reconciled</div>
                  <p className="text-slate-400 line-clamp-1">Banking UPI gateway RRN confirmed and credited.</p>
                </button>
                ...
                <button
                  ...
                  className="text-left p-2 rounded-xl bg-slate-950 hover:bg-slate-800/80 border border-slate-800 hover:border-indigo-500/50 text-[10px] text-slate-300 transition-all space-y-0.5"
                >
                  <div className="font-bold text-cyan-400">🔬 Material Grading Assay Accord</div>
                  <p className="text-slate-400 line-clamp-1">Physical spectrometry audit resolved rate classification.</p>
                </button>
                ...
                <button
                  ...
                  className="text-left p-2 rounded-xl bg-slate-950 hover:bg-slate-800/80 border border-slate-800 hover:border-indigo-500/50 text-[10px] text-slate-300 transition-all space-y-0.5"
                >
                  <div className="font-bold text-rose-400">📋 Permissible Tolerance Dismissal</div>
                  <p className="text-slate-400 line-clamp-1">Weight within legal margin, facility weighment upheld.</p>
                </button>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full p-3 bg-slate-950 border border-slate-700 rounded-2xl text-white text-xs focus:outline-none focus:border-indigo-500 transition-all leading-relaxed"
              />
            </div>
