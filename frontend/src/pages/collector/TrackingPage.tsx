    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-20">
      ...
      {loading ? (
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-12 text-center text-slate-400 space-y-3">
          ...
          <p className="text-xs">{language === 'hi' ? 'ट्रैसेबिलिटी डेटा लोड हो रहा है...' : language === 'mr' ? 'ट्रॅकिंग लोड होत आहे...' : 'Loading traceability ledger...'}</p>
        </div>
      ) : !lot ? (
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center text-slate-400 space-y-3">
          <p className="text-sm font-bold text-slate-300">
            {language === 'hi' ? 'लॉट आईडी नहीं मिली।' : language === 'mr' ? 'लॉट क्रमांक आढळला नाही.' : 'Lot ID not found.'}
          </p>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            {language === 'hi' ? 'कृपया ऊपर दिए गए पंजीकृत लॉट में से चुनें या एक मान्य लॉट आईडी दर्ज करें।' : language === 'mr' ? 'कृपया वरीलपैकी अचूक क्रमांक निवडा.' : 'Please select from your registered lots above or enter a valid lot identifier.'}
          </p>
          {allLots.length > 0 && (
            <button
              type="button"
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-emerald-400 text-xs font-bold rounded-xl border border-slate-700 active:scale-95 transition-all shadow"
            >
              <span>{language === 'hi' ? 'नवीनतम लॉट देखें' : language === 'mr' ? 'नवीनतम लॉट पहा' : 'View Latest Lot'}</span>
              <span className="font-mono">({allLots[0].id})</span>
            </button>
          )}
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xl space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
              <div>
                <span className="font-mono text-xs font-black text-emerald-400 tracking-wider block">{lot.id}</span>
                <h2 className="text-xl font-black text-white leading-tight mt-0.5">{getCategoryLabel(lot.materialCategory, language)}</h2>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  ...
                  className="px-3.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-emerald-300 border border-slate-700 rounded-xl text-xs font-black flex items-center gap-1.5 shadow active:scale-95"
                >
                  <FileText className="w-3.5 h-3.5 text-emerald-400" />
                  <span>{language === 'hi' ? 'CPCB Form-6 मेनिफेस्ट' : language === 'mr' ? 'CPCB Form-6 मॅनिफेस्ट' : 'CPCB Form-6 Manifest'}</span>
                </button>

                <span className="px-3 py-1.5 bg-emerald-950 text-emerald-300 text-xs font-black rounded-xl border border-emerald-800">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 text-xs">
              <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800">
                ...
                <span className="font-black text-white truncate block mt-0.5">
              ...
              <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800">
                ...
                <span className="font-black text-amber-300 text-xs truncate block mt-0.5">
              ...
              <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800 col-span-2 sm:col-span-1">
                ...
                <span className="font-black text-white block mt-0.5 truncate">
            ...
            <div className="pt-4 border-t border-slate-800/80 space-y-3">
              <div className="flex items-center justify-between text-xs font-bold text-slate-400">
                ...
                <span className="text-emerald-400 font-extrabold bg-emerald-950/80 px-2.5 py-0.5 rounded-full border border-emerald-800 text-[11px]">
              ...
              <div className="relative pt-2 pb-1">
                ...
                <div className="absolute top-6 left-[10%] right-[10%] h-1 bg-slate-800 -translate-y-1/2 z-0 rounded-full">
                      ...
                      <div key={stage.key} className="flex flex-col items-center gap-1.5 text-center px-0.5">
                        <div
                          className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-black transition-all ${
                            isCurrent
                              ? 'bg-emerald-500 text-slate-950 ring-4 ring-emerald-500/30 shadow-lg shadow-emerald-950 scale-110'
                              : isCompleted
                        ...
                        <span className={`text-[10px] font-black leading-tight block ${
                          isCurrent ? 'text-emerald-400 font-extrabold' : isCompleted ? 'text-slate-200' : 'text-slate-500'
                        }`}>
                          {stage.label}
                        </span>
                      </div>
              ...
              {timeline.map((item, idx) => {
                ...
                return (
                  <div key={item.id || idx} className="relative flex items-start gap-4 pl-1">
                    <div className="w-7 h-7 rounded-full bg-emerald-600 text-white flex items-center justify-center text-xs font-bold shadow shrink-0 z-10 ring-4 ring-slate-900">
                      ✓
                    </div>
                    <div className="bg-slate-950 border border-slate-800 p-4 rounded-2xl flex-1 space-y-2.5 shadow-sm">
                      <div className="flex items-center justify-between flex-wrap gap-1">
                        <h4 className="font-black text-sm text-white">{item.title}</h4>
                        <span className="text-[11px] font-mono text-emerald-400 bg-slate-900 px-2 py-0.5 rounded-md border border-slate-800">
                      ...
                      {item.eventHash && (
                        <div className="bg-slate-900/90 p-2.5 rounded-xl border border-slate-800/80 font-mono text-[10px] text-slate-400 space-y-1">
                          <div className="flex items-center gap-1.5 truncate">
                            <Key className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                            <span className="text-slate-500 font-bold">Hash:</span>
                            <span className="text-emerald-400 truncate font-mono">{item.eventHash}</span>
                          </div>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-slate-900 text-[11px] text-slate-400 flex-wrap gap-2">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-lg w-full shadow-2xl space-y-5 text-white max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Manifest Number</span>
                <span className="font-mono text-emerald-400 font-bold">CPCB-MAN-{lot.id.replace('EW-', '')}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Generated At</span>
                <span className="text-slate-200 font-mono text-[11px]">{formatAuditDate(lot.createdAt)}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">E-Waste Category</span>
                <span className="text-white font-bold">{getCategoryLabel(lot.materialCategory, language)}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Scaled Weight</span>
                <span className="text-emerald-400 font-bold">{lot.approxWeight} kg ({lot.condition})</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs">
              <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-emerald-400 font-bold text-[10px] uppercase tracking-wider block">Consignor (Collector)</span>
                <p className="font-bold text-white text-sm">{lot.collectorName}</p>
                <p className="text-[11px] text-slate-400">{lot.locationDistrict}, {lot.locationState}</p>
                <p className="text-[10px] text-slate-500 font-mono">KYC: CPCB-REG-COLL-98</p>
              </div>

              <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800 space-y-1">
                <span className="text-emerald-400 font-bold text-[10px] uppercase tracking-wider block">Consignee (CPCB Recycler)</span>
                <p className="font-bold text-white text-sm truncate">{recycler?.facilityName || 'GreenEarth E-Waste Solutions Pvt Ltd'}</p>
                <p className="text-[11px] text-slate-400 font-mono">{recycler?.registrationNo || 'CPCB/EWR/UP/LKO/2023/8812'}</p>
                <p className="text-[10px] text-slate-500">Authorized Capacity: 5,400 MTA</p>
              </div>
            </div>
